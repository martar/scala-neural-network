...
class KohonenNetwork(layers: List[Layer]) extends Network(layers, false) {
  ...
  /*
   * alfa jest stałą uczenia, wybieraną na ogół pomiędzy 0,1 i 0,7. 
   * Dzięki temu lekko "popychamy" wektor wag w kierunku wektora danych. 
   * Algorytm ten wykazuje niekiedy rosnącą niestabilność, dlatego często stosuje się obecnie metodę subtraktywną. 
   * Opiera się ona na spostrzeżeniu, że pewnym sposobem lekkiego przyciągania 
   * wektora wag w kierunku wektora wejściowego jest odjąć je, a następnie część tej różnicy dodać do wektora wag
   */
